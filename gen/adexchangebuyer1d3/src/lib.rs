...
//! This documentation was generated from *Ad Exchange Buyer* crate version *1.0.12+20190614*, where *20190614* is the exact revision of the *adexchangebuyer:v1.3* schema built by the [mako](http://www.makotemplates.org/) code generator *v1.0.12*.
//! 
...
impl<'a, C, A> DirectDealListCall<'a, C, A> where C: BorrowMut<hyper::Client>, A: oauth2::GetToken {
    ...
    /// req = req.header(header::Range::Bytes(vec![header::ByteRangeSpec::FromTo(10,20)]));
    /// ```
    pub fn header<H>(mut self, header: H) -> DirectDealListCall<'a, C, A>
                                                        where H: hyper::header::Header+hyper::header::HeaderFormat {
        self._additional_headers.set(header);
        self
    }
    ...
    /// encompass more rights than others. For example, for listing resources, a *read-only* scope will be
    /// sufficient, a read-write scope will do as well.
    pub fn add_scope<T, S>(mut self, scope: T) -> DirectDealListCall<'a, C, A>
                                                        where T: Into<Option<S>>,
                                                              S: AsRef<str> {
        match scope.into() {
          Some(scope) => self._scopes.insert(scope.as_ref().to_string(), ()),
...
impl<'a, C, A> DirectDealGetCall<'a, C, A> where C: BorrowMut<hyper::Client>, A: oauth2::GetToken {
    ...
    /// req = req.header(header::Range::Bytes(vec![header::ByteRangeSpec::FromTo(10,20)]));
    /// ```
    pub fn header<H>(mut self, header: H) -> DirectDealGetCall<'a, C, A>
                                                        where H: hyper::header::Header+hyper::header::HeaderFormat {
        self._additional_headers.set(header);
        self
    }
    ...
    /// encompass more rights than others. For example, for listing resources, a *read-only* scope will be
    /// sufficient, a read-write scope will do as well.
    pub fn add_scope<T, S>(mut self, scope: T) -> DirectDealGetCall<'a, C, A>
                                                        where T: Into<Option<S>>,
                                                              S: AsRef<str> {
        match scope.into() {
          Some(scope) => self._scopes.insert(scope.as_ref().to_string(), ()),
